...
def plot_transactions_distribution(transactions: pd.DataFrame):
    ...
    sample_size = min(10000, len(amount_val))  # Limit the sample size to the population size
    ...
    sns.distplot(time_sample / 86400, ax=ax[1], color='b', bins=100, hist=True, kde=False)
...
def plot_transactions_daily_stats(transactions: pd.DataFrame):
    ...
    tx_stats['stat_type'] = ["nb_tx_per_day"] * n_days + ["nb_fraud_per_day"] * n_days + [
